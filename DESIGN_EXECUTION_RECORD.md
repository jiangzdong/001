...
- 已完整读取全局设计规范：是
- 产品目标：双屏站点终端将主咨询软件全屏落在用户的外接扩展屏；虚拟长者测试时另一块屏显示全屏测试控制台。
- 核心用户与陪同用户：站点使用者与值守/测试人员。
- 平台、设备、视口、观看距离：macOS Electron；实测主屏 2880×1864，外接 AU16T 1920×1200，双屏独立扩展；保留外接屏高度驱动的 9:16 终端画布。
- 输入方式与使用环境：站立式触控/麦克风站点终端；测试人员可在主屏查看控制台。
- 成功指标 / 失败定义：外接显示器无论当前横竖方向都优先承载咨询终端且全屏；双屏测试控制台占另一屏全屏。回退主屏、小窗或拉伸人物均为失败。
- 事实源优先级：用户最新双屏要求 > 当前 macOS 显示器拓扑实测 > 既有 9:16 终端视觉来源 > 当前 Electron 启动实现。
- 当前最小 skill 集：design-taste-frontend（既有产品视觉保护与可读性审查）；不新增页面或视觉资产。
- 实际调用证据：已完整读取 `design-taste-frontend/SKILL.md`；其 Design Read 和保留/增强/删除/重做已同步至 `design-qa.md`。
- 用户选择：用户明确要求“软件界面高度按照我的扩展屏设计 双屏状态下需要全屏”；采用外接屏优先且保持现有竖向终端画布。
- Token、组件与状态：不修改渲染层 Token 或组件；仅 BrowserWindow 的显示器选择、完整 bounds 和全屏状态。
- 明确反例：不以“仅竖屏”作为外接显示器唯一条件；不将数字人横向拉伸填满横屏；不新增单侧状态描边。
- 空/加载/错误/权限/取消/恢复：单屏或未发现外接屏时回退主屏；`--windowed` 仍用于开发小窗；窗口关闭沿用既有生命周期，不改变语音、权限或对话恢复。
- 响应式、触控与减少动态效果：9:16 内容画布按显示器高度居中，现有触控尺寸、对焦和减少动态规则不变。
- 保留：现有冷白医疗蓝、数字人比例、9:16 画布、无标题栏全屏与主屏/测试控制台职责。
- 增强：外接屏优先规则、横向外接屏可用、双屏测试控制台完整占屏。
- 删除：横向外接屏回退主屏和控制台浮动小窗。
- 重做：无 UI 表面、动效、图标、页面或交互流程重做。
- `PRE_CODE_GATE`: PASS
- 核心流程操作证据：在真实 macOS 双屏拓扑上，启动代码以非主显示器优先并传入该显示器完整 bounds；主窗口保留无标题栏全屏，虚拟长者控制窗取另一个显示器 bounds 后进入全屏。
- 异常、取消与恢复证据：无外接屏时 `externalDisplays` 为空，安全回退到竖屏主显示器或 primary；`--windowed` 仍保留开发调试出口，窗口关闭路径未变。
- 目标视口截图：既有 9:16 产品画布为视觉来源；新目录包已由桌面自动化真实启动并读取首屏，数字人、欢迎信息、设置入口和底部语音输入舱完整可见；当前自动化通道仅能证明可视首屏，不能导出窗口所属物理显示器坐标。
- 可访问性与焦点检查：窗口改动未涉及输入、焦点、读屏、语音权限或触控控件；现有无标题栏与全屏不删除应用内设置入口。
- 控制台 / 溢出 / 横向滚动：本轮不修改网页布局；主咨询画布继续按扩展屏高度，避免横向拉伸人物或聊天内容。
- 自动测试与构建：Electron CJS 语法检查通过；启动/显示器/全屏专项与现有 Station Advisor、Harness、Virtual Senior 回归 69/69 PASS；Vite 4585 modules transformed，Sites 后处理 PASS。
- Taste：保留 / 增强 / 删除 / 重做：保留 9:16 终端与冷白蓝；增强外接屏优先与完整占屏；删除主屏错误回退/控制台小窗；无视觉重做。
- Audit：阻断 / 高 / 中 / 低 / 通过项：阻断 0、高 0；中项为新包在用户的物理外接屏上需最终目视确认；低项为横向扩展屏周围不强制填满内容区以避免数字人失真；通过项为显示器选择、全屏声明和回退逻辑。
- 回退点：现有 `release/multi-turn-v1.5.23/mac-arm64` 目录包及不改动本轮前的启动逻辑均保留；新候选使用独立输出目录。
- 提交 / 产物：本轮未提交；已生成独立 macOS arm64 目录包 `release/external-display-v1.5.23/mac-arm64/小安站点咨询顾问 V1.5.23.app`，包内 `app.asar` SHA-256 为 `7249c017df633601266445ad5a5f593902e3f7365bc3961ab88dc0bd94a5657d`，不覆盖既有 V1.5.23 候选。
- 未验收范围：Windows 2400×3840、实体触控、真实 ASR/TTS/口型、生产 MCP 与用户最终物理观看距离不包含在本轮。
- 最终结论：启动逻辑、构建、目录包及首屏实际启动均通过当前窄范围修复门槛；新包已优先选择外接屏代码路径，仍需用户在 AU16T 上目视确认实际落位。
- `PRE_DELIVERY_GATE`: PASS
- 通过证据：当前视觉来源为既有 9:16 产品画布；系统 `SPDisplaysDataType` 已确认真实双屏独立扩展；启动逻辑专项静态回归 69/69 PASS。
- 2026-09-04 网页语音可用性修复：本机网页预览曾无 Vite 服务监听，恢复服务后 `/api/speech/status` 返回 `ready:true`、`offline:true`（SenseVoice Small INT8）。同时移除浏览器原生识别必须带隐藏 `allowWebSpeech=1` 查询参数的限制：网页优先本地 SenseVoice，服务临时不可用时再允许浏览器在用户授权麦克风后回退。无视觉组件、布局或权限语义改动，`DESIGN_VARIANCE=0`、`MOTION_INTENSITY=0`、`VISUAL_DENSITY=0`；Station Advisor + desktop 静态回归 36/36、Vite 4585 modules、Sites 后处理均通过。物理麦克风输入仍需用户授权/声学验证。

## 14. 2026-09-04 模拟姓名、必测语音与可选择/重测场景（当前增量）

- 用户要求：生成模拟姓名；语音不可略过；受阻及未通过有特殊样式；测试记录可重测；22 项全选、多选、单选。
- 范围判断：增加场景多选和语音执行/重测交互，属于新增设计范围，不套用窄范围例外。前次方案 1 和 V1.5.23 实际截图是已有视觉来源，但不自动代替本次门禁。
- 已询问：是否授权沿用方案 1，豁免本轮重新展示三套方向稿；尚无明确答复，不将请求当作授权。暂仅推进数据和后台，不编辑 UI。
- 保留要求：左控右看；姓名标为合成；语音识别/合成/播放分别记录真实结果；受阻或失败用表面色、图标及文字，不用单侧描边；重测创建新报告并保留来源；相关前置轮次透明展示。
- `PRE_CODE_GATE`: FAIL
- `PRE_DELIVERY_GATE`: FAIL
- 当前证据：仅后台候选及297自动测试通过，真实模型推理诊断最新16/22；具体见 docs/VIRTUAL-SENIOR-VOICE-SELECTION-20260904.md。GUI语音接入、状态样式、控件、实际交互、截图、构建与打包均未完成。pre-code验证器实际FAIL；未编辑UI、未提交/推送，V1.5.23 / d377f23 保留用于回退。
...
- 2026-09-04 双屏扩展屏落位修正：系统显示拓扑实测为主屏 `2880×1864`、外接 AU16T `1920×1200`（当前横向）。原实现只筛选“高大于宽”的显示器，横向外接屏会错误回退主屏。本轮将主咨询终端改为双屏时优先外接显示器（竖屏优先、横屏外接也可），按其完整 bounds 全屏；虚拟长者双屏测试时，控制台占用另一块屏幕并全屏。保持既有 9:16 内容画布按扩展屏高度居中，不拉伸人物或新增界面。`DESIGN_VARIANCE=0`、`MOTION_INTENSITY=0`、`VISUAL_DENSITY=0`；当前真实显示器拓扑与既有终端画布为事实源，属于启动落位缺陷修正，适用窄范围例外。
...
## 13. 2026-09-06 模拟姓名、22 项选择、必测语音与记录重测交付

- 本节只收口用户在方案 1 之后追加的测试产品需求；`PRE_CODE_GATE=PASS`，用户选定稿仍为 `docs/design/virtual-senior-live-observer/option-1.png`，SHA-256 `b535675faeae1d42260b4886a4f56b9f0da74b0b08fd22add3c9d4a13083ee16`。12 项实际查看参考和转化分析位于 `DESIGN_RESEARCH.json`，不混用方案 2/3。
- 实施范围：10,000 人确定性中文模拟姓名；22 项全选/单选/多选与前置依赖；每轮测试问题本机 TTS→实际 AudioContext 播放→本机 ASR→Harness；回答音频本机合成与实际播放；失败/受阻完整表面样式、原因和逐轮表格；历史记录按同居民/同场景/同轮次生成新 run 重测并保留来源。
- 源码真实交互：`QA-EXTERNAL/virtual-senior-community/live-voice-ui-20260906/source-final2/live-voice-ui-report.json` PASS。1440×1024 覆盖未选择、选择、运行、通过详情和停止后受阻重测；750×1200 无横向溢出。单侧线条 0、低于 44px 控件 0、console error 0。首次运行发现轮次面板遮挡首屏停止按钮，已改为启动/重测自动收起并用最终截图复验。
- 同屏比较：`QA-EXTERNAL/virtual-senior-community/live-voice-ui-20260906/source-final2/design-comparison-option1-vs-current.png`。实现保留选定稿的信息层级、双栏任务台、医疗蓝和安静表面；概念人像由现有产品数字人资产替换，演示数据由真实合成数据/运行状态替换。
- 独立打包：版本提升为 V1.5.24，包在 `release/live-voice-v1.5.24/mac-arm64/小安站点咨询顾问 V1.5.24.app`，`CFBundleShortVersionString=1.5.24`，`app.asar` SHA-256 `a43d72d19fdf07e7a29b0c8ac7f58d46258741747987c379c45af5d95328a768`；旧 V1.5.23 包和远端提交 `d377f23` 保留。打包 App 从普通 GUI 入口执行同一验收，`QA-EXTERNAL/virtual-senior-community/live-voice-ui-20260906/packaged-v1.5.24/live-voice-ui-report.json` PASS。
