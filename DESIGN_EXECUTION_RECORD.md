...
- 当前阶段：阶段 G 已完成本机实现与独立验收；当前为条件通过，等待 Windows 2400×3840 实机和当前候选 Git 标识。
- 新项目或新设计范围：是。
...
- 自动测试与构建：统筹当前源码独立复跑完整 Node **245/245 PASS**；Vite **4582 modules transformed**；Sites **4/4 PASS**；相关语法与禁用缺失 LFS filter 的只读 `git diff --check` 通过。
...
- 提交 / 产物：当前 V1.5.19 源码与 QA 报告仍在未提交工作树；社区 v1.3 两份确定性数据、独立 sweep、Electron 主流程、生命周期和视觉报告均位于 `QA-EXTERNAL/virtual-senior-community/`，不进入生产包。
- 未验收范围：Windows 2400×3840 实体终端触屏/观看距离、系统辅助技术、当前候选 commit/push/package，以及真实生产 MCP、ASR/TTS/Viseme/V34 链路。
