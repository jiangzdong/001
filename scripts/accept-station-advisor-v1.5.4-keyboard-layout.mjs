...
const outputDir = path.resolve(process.argv[3] || "qa/station-advisor-v1.5.4-keyboard-layout-electron-dev");
const cdpBase = `http://127.0.0.1:${port}`;
const reportPath = path.join(outputDir, "station-advisor-v1.5.4-keyboard-layout-report.json");
...
const report = {
  suite: "station-advisor-v1.5.4-keyboard-layout-electron",
...
try {
  ...
  if (report.initial.title !== "小安站点咨询顾问 V1.5.4") report.failures.push(`unexpected-title:${report.initial.title}`);
  if (report.runtime?.packaged !== true) report.failures.push("runtime-not-packaged");
  if (report.runtime?.version !== "1.5.4") report.failures.push(`unexpected-runtime-version:${report.runtime?.version || "missing"}`);
