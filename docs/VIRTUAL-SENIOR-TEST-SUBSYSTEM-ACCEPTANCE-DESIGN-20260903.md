...
方案 C 仍适合当前项目：它把可重复的发布门禁建立在固定 case 和正式 Harness/MCP 协议之上，同时为后续 LLM 表达变体与媒体门禁保留扩展点。V1.5.18 已完成 M0 验收内核和 M1 受控入口的首版：3 个合成画像、10 个固定场景、真实 Streamable HTTP Fixture MCP、Harness 硬断言、单项/批次/停止、JSON 持久化报告、跨重启趋势恢复及统计分析 UI。2026-09-03 的 M2a 本机候选已新增严格 Schema 的合成语言变体、显式 `synthetic-test-only` 数据分类、不可变哈希制品、重放模式和生成维度统计；硬 oracle 与 Tool 白名单仍只读自固定场景。固定 WAV ASR、真实 TTS/Viseme/V34、性能与 Windows 2400×3840 媒体门禁仍未实现，M2 尚不能整体判定完成。

## 15. 社区级全量测试集范围变更

2026-09-03 用户明确指出 3 个画像不足，并要求画像数据横向覆盖当前站点数字人全部业务场景与 5 MCP / 16 Tool 所需接口数据，纵向达到至少一个社区的用户体量，作为全量测试集。因此原 3 画像/10 场景只保留为兼容性冒烟基准，不再代表目标全量覆盖。

新目标以 `docs/VIRTUAL-SENIOR-COMMUNITY-DATASET-SPEC-20260903.md` 为数据集规格：默认 `community-full` 为不少于 10,000 名纯合成长者、约 350 万级跨域记录，覆盖全部 Tool 的成功、空数据、缺失、过期、权限、分页、超时、合同损坏和幂等边界；同一居民跨身份、健康、会员和站点服务数据必须引用一致。当前 Fixture 仅 9 个 Tool 有具体样例、其余可能退化为通用 `{ok:true}`，故社区级全量数据集当前状态为**未实现**，不得因 3 画像批次通过而标记完成。
