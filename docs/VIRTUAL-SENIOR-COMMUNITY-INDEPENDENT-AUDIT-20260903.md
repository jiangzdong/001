...
统筹最终独立判定为 `CONDITIONAL`：V1.5.19 的社区级纯合成数据、5 MCP / 16 Tool 合同、全量 resident sweep、持久化作业控制、统计下钻和 macOS Electron 等价环境均已通过当前源码复验，候选提交 `269ce3f` 已推送远端；Windows 2400×3840 实体终端仍缺证据，因此不得写成全面交付或正式生产 MCP 通过。历史首轮、第二轮失败与每次退回原因继续保留在下文，不把旧失败覆盖成一次成功。
...
| G12 可追溯与回退 | PASS | 数据版本、生成器、seed、hash、命令和远端回退基线齐全；V1.5.19 功能候选提交 `269ce3f` 已推送至 `origin/feat/station-advisor-local-demo`。 |

## 剩余放行条件

在 Windows 2400×3840 目标机复跑当前 Electron 视觉/触控/焦点门禁后，才能把 G10 改为 PASS。真实生产 MCP、ASR/TTS/Viseme/V34 和实体硬件不属于本次纯合成社区数据全量扫描的通过范围，仍按项目总门禁单独验收。
