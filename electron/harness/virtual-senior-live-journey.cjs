...
const JOURNEY = Object.freeze([
  ...
  step("service-detail", "服务追问", "刚才列出的第一项服务，开放时间和预约要求是什么？", `${S}.get_station_service_detail`, { dependsOn: "services" }),
  step("knowledge", "站点知识", "服务的使用说明，在哪里？", `${C}.search_station_knowledge`),
  step("activities", "站点活动", "再看看站点有哪些活动。", `${C}.list_station_activities`),
  step("activities-next", "活动翻页", "下一页，还有活动吗？", `${C}.list_station_activities`, { dependsOn: "activities" }),
  ...
  step("level", "会员等级", "会员等级是什么？有哪些权益？", `${M}.get_member_level`),
  step("recharge", "充值记录", "查看我的充值记录。", `${M}.list_recharge_records`),
  step("recharge-next", "充值追问", "充值记录，请看下一页。", `${M}.list_recharge_records`, { dependsOn: "recharge" }),
  ...
  step("vitals", "最新体征", "最新健康体征，是什么时候记录的？", `${H}.get_latest_health_labels`),
  step("history", "半年体征", "查看近半年健康体征的记录。", `${H}.get_indicator_evidence`),
  ...
  step("save", "合成结果保存", "我确认。仅在合成测试区保存测试草稿。依据刚才的证据。不作诊断。", `${H}.save_risk_assessment_result`, { dependsOn: "history" }),
...
function coverageFor(turns, events, planned) {
  const businessStatus = (turn) => turn.businessStatus || turn.status;
  const tools = MCP_TOOL_CATALOG.filter(([server, name]) => planned.some((s) => s.tool === `${server}.${name}`)).map(([server, name, title]) => {
    ...
    return { tool, server, title, calls, successes, blocked: selected.filter((t) => ["denied", "auth_required"].includes(businessStatus(t))).length, failed: selected.filter((t) => businessStatus(t) === "failed").length, skipped: selected.filter((t) => businessStatus(t) === "skipped").length };
  });
