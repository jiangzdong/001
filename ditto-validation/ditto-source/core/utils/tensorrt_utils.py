...
# Python 3.8+ no longer searches PATH automatically for extension-module DLLs.
# Keep the directory handles alive for the lifetime of the process so the local
# TensorRT SDK and PyTorch's CUDA 11.8 runtime can be used without a global CUDA
# Toolkit installation.
_dll_directory_handles = []
if os.name == "nt" and hasattr(os, "add_dll_directory"):
    dll_directories = [
        os.path.join(os.environ.get("TENSORRT_ROOT", ""), "lib"),
        os.path.join(os.path.dirname(torch.__file__), "lib"),
    ]
    os.environ["PATH"] = os.pathsep.join([*(path for path in dll_directories if path), os.environ.get("PATH", "")])
