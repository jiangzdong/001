...
class StreamSDK:
    ...
    def setup(self, source_path, output_path, **kwargs):
        ...
        # ======== Prepare Options ========
        self.frame_callback = kwargs.pop("frame_callback", None)
        self.emit_initial_context = kwargs.pop("emit_initial_context", False)
        ...
        self.writer = VideoWriterByImageIO(self.tmp_output_path) if self.tmp_output_path else None
        self.writer_pbar = tqdm(desc="writer") if self.writer else None
    ...
    def _writer_worker(self):
        while not self.stop_event.is_set():
            ...
            if self.frame_callback is not None:
                self.frame_callback(res_frame_rgb)
        ...
        while not self.stop_event.is_set():
            ...
            while True:
                ...
                if res_kp_seq_valid_start is None:
                    ...
                    if not self.emit_initial_context:
                        local_idx += real_valid_len
                        global_idx += real_valid_len
                        continue
                    valid_res_kp_seq = res_kp_seq[:, :real_valid_len]
                    # The common cursor advance below should leave the next
                    # unread window at valid_clip_len, not at twice that value.
                    res_kp_seq_valid_start = 0
                else:
    ...
    def close(self):
        ...
        try:
            if self.writer is not None:
                self.writer.close()
            if self.writer_pbar is not None:
